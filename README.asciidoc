...
This is an continuous effort towards enabling automatic support for rxruting SPARQL queries over Graph systems via Gremlin query language by converting sparql queries to gremlin traversals/queries preserving its semantics. 
...
2. enable support for Order-By queries [Done-testing phase]

3. enable support for Group-By queries [Done-testing phase]

4. adding support for ASK queries [Pending]

5. enable support (translation) of QALD 6 dataset (SPARQL queries over DBpedia) [work in progress]

6. enable support (translation) for Northwind dataset (SPARQL queries over Northwind dataset [property-graph] ) [Done]

7. enable support for dataset independent query translation [work in progress] (This is allow a user to execute SPARQL queries over almost any dataset without being bothered about the internal mappings and configuration settings)
