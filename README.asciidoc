...
An effort towards converting sparql queries to gremlin traversals/queries. In this work we started with an aim to extend the work by Daniel Kuppitz [https://github.com/dkuppitz/sparql-gremlin]; However, towards the end we landed up modifying the core and built it from almost scratch! But we like to give due credits for getting us started :)

##The extentions are listed as follows:

1. adding support for Union queries [Done]

2. adding support for Order-By queries [Done]

3. 2. adding support for Group-By queries [Done]

4. adding support for ASK queries [Pending]

5. Support translation for QALD 6 dataset (SPARQL queries over DBpedia) [planned]

6. Support translation for Northwind dataset [Partially]
